...
const NAV_LINKS = [
  ...
  { to: '/student/timetable/internal',   label: 'Internal Timetable',icon: Calendar },
  { to: '/student/timetable/external',   label: 'External Timetable',icon: Building },
  { to: '/student/attendance',          label: 'Attendance Summary',icon: CheckSquare },
...
export default function StudentLayout({ children }) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-dark-900 border-r border-white/5 portal-sidebar">
      ...
      <div className="flex items-center gap-3 px-6 py-5 border-b border-white/10">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500 to-rose-600 flex items-center justify-center flex-shrink-0">
          <GraduationCap size={20} className="text-white" />
        </div>
        <div>
          <p className="font-display font-bold text-white text-base leading-none">TMS</p>
          <p className="text-[10px] text-gray-500 mt-0.5 uppercase tracking-widest">Student Portal</p>
        </div>
      </div>
      ...
      <div className="px-3 py-4 border-t border-white/10">
        <div className="glass-card p-3 flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-rose-600 flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-gray-500 text-[10px] truncate">{user?.registerNumber ?? ''}</p>
          </div>
          <button
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-dark-900 student-layout">
      {/* Desktop Sidebar */}
      ...
      <div className="flex-1 flex flex-col overflow-hidden portal-body-light">
        ...
        <header className="flex items-center justify-between h-16 px-6 bg-dark-800/60 backdrop-blur-md border-b border-white/10 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 lg:hidden">
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-purple-500 to-rose-600 flex items-center justify-center">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              <p className="text-white text-sm font-medium leading-none">{user?.name ?? 'Student'}</p>
              <p className="text-gray-500 text-xs mt-0.5">{user?.department?.code ?? 'Student'}</p>
            </div>
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-rose-600 flex items-center justify-center">
              <span className="text-white text-xs font-bold">
                {user?.name?.charAt(0)?.toUpperCase() ?? 'S'}
              </span>
            </div>
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-y-auto p-6 bg-dark-900/50">
          {children || <Outlet />}
        </main>
      </div>
    </div>
